...
-- ISP Session 6.5/6.0; creating these tables on temp
-- disables effectively logging, which is a huge performance gain
CREATE PROC [dbo].[CreateTempTables]
AS
BEGIN

  	EXEC dbo.FixSecurityForTempDb
	IF EXISTS(SELECT name from tempdb.sys.sysobjects where name ='tblSessions_ISP') RETURN;
	CREATE TABLE [tempdb].[dbo].[tblSessions_ISP]
		(
	...
	CREATE TABLE [tempdb].[dbo].[tblIspSession_Stats]
	(	statid int NOT NULL,
		APP_KEY binary(16) NOT NULL,
	...
	ALTER TABLE [tempdb].[dbo].[tblIspSession_Stats] WITH NOCHECK  ADD 
		CONSTRAINT  PK_tblIspSession_Stats PRIMARY KEY CLUSTERED 
		(statid , APP_KEY ) 

	ALTER TABLE [tempdb].dbo.tblSessions_ISP ADD 
	...
	CREATE TABLE [tempdb].[dbo].[tblPageRequestTime](
		[GUID] [binary](16) NOT NULL,
	...
	CREATE NONCLUSTERED INDEX [IX_tblPageRequestTime] ON [tempdb].[dbo].[tblPageRequestTime]
	(
		[GUID] ASC
	) WITH (FILLFACTOR = 100)
...
CREATE PROCEDURE [dbo].pSessionFetch
(
	@App_Key binary(16),
	@GUID binary(16)
...
SET NOCOUNT ON;
	 SELECT	[GUID], 
			...
			[timest],
			[State]
	 FROM [tempdb].[dbo].tblSessions_ISP 
	WHERE [GUID] = @GUID 
		AND [App_Key] = @App_Key;
GO

GO
CREATE PROCEDURE [dbo].FetchAllTimeOut
AS
	SET NOCOUNT ON;
		SELECT [APP_KEY], [GUID]
	FROM [tempdb].[dbo].TBLSESSIONS_ISP (NOLOCK) 
	WHERE DATEDIFF(minute, LastUpdated, GetDate()) > Expires
GO


CREATE PROCEDURE [dbo].pSessionInsert
	@App_Key binary(16), 
	@GUID binary(16), 
	@Expires int, 
	@ReEntrance bit, 
...
SET @defdate = GetDate();
	...
	IF NOT EXISTS(SELECT GUID FROM [tempdb].[dbo].tblSessions_ISP WHERE [GUID]=@GUID AND [App_Key] = @App_Key)
	INSERT INTO [tempdb].dbo.tblSessions_ISP
...
GO
	...
	CREATE PROCEDURE [dbo].[pSessionSave]
	(
		@App_Key binary(16),
		@GUID binary(16),
	...
	AS
		...
		IF @StateEnc IS NULL BEGIN
			If @zlen = 0 OR @zLen IS NULL BEGIN
				IF @GUIDNew IS NULL  
					UPDATE [tempdb].dbo.tblSessions_ISP
		...
		if (NOT @pageRequestTime IS NULL) BEGIN
			INSERT tempdb.dbo.tblPageRequestTime ([GUID], pageRequestTime) 
...
CREATE PROCEDURE [dbo].[pSessionGetStats]
	@App_Key binary(16) ,
	@GUID binary(16) = NULL
AS
BEGIN
	...
	IF (NOT @GUID IS NULL) BEGIN
		...
		SELECT @AverageRequestTime = AVG(P.PageRequestTime) , 
			...
			@TotalRequests = COUNT(*) 
		FROM [tempdb].[dbo].tblPageRequestTime AS P
		WHERE P.[GUID] = @GUID;
		...
		FROM 
			[tempdb].[dbo].tblIspSession_Stats AS S	
		WHERE 
...
CREATE PROCEDURE [dbo].[pSessionUpdateStats]
AS
BEGIN
SET NOCOUNT ON;
	...
	FOR 	SELECT APP_KEY
		FROM [tempdb].[dbo].tblSessions_ISP GROUP BY APP_KEY;
	...
	WHILE (1=1) BEGIN
		...
		IF NOT EXISTS (SELECT statid FROM [tempdb].[dbo].[tblIspSession_Stats]
					WHERE statid=1 AND APP_KEY = @App_Key) BEGIN

			INSERT INTO [tempdb].[dbo].[tblIspSession_Stats]
		...
		@CountExpired = ISNULL(
						SUM(
								...
								ELSE 0 END), 0)
		FROM [tempdb].[dbo].TBLSESSIONS_ISP AS I		
...
--		PRINT @CountExpired;
		...
		UPDATE [tempdb].[dbo].[tblIspSession_Stats]
...
CREATE PROCEDURE [dbo].pSessionDelete
(
	@App_Key binary(16),
	@GUID binary(16)
)
AS	
	DELETE [tempdb].[dbo].tblSessions_ISP
	WHERE [GUID] = @GUID
	AND App_Key = @App_Key;
	DELETE [tempdb].[dbo].tblPageRequestTime
	WHERE [GUID] = @GUID
...
CREATE procedure [dbo].[proc_DeleteSessions] AS
BEGIN
	...
	DECLARE ispsession_cursor CURSOR LOCAL FAST_FORWARD FOR
	  SELECT APP_KEY, GUID FROM [tempdb].[dbo].TBLSESSIONS_ISP WHERE 
...
CREATE PROCEDURE [dbo].SessionPing
	@App_Key binary(16),
	@GUID binary(16),
	@updateTo datetime	
AS
BEGIN	
	SET NOCOUNT ON;
	UPDATE [tempdb].[dbo].tblSessions_ISP SET LastUpdated=@updateTo
...
-- ================================================
CREATE PROCEDURE [dbo].[FixSecurityForTempDb]	
AS
BEGIN
