...
{
    static internal class FindPrivateKey
    {
        ...
        public static string GetKeyFileName(X509Certificate2 cert)
        {
            ...
            {                
                ...
                try
                {
                    if (CryptGetProvParam(hProvider, CryptGetProvParamType.PP_UNIQUE_CONTAINER, null, ref cbBytes, 0))
                    {
                        keyFileBytes = new byte[cbBytes];
                        if (CryptGetProvParam(hProvider, CryptGetProvParamType.PP_UNIQUE_CONTAINER, keyFileBytes, ref cbBytes, 0))
                        {
                            // Copy eveything except tailing null byte
                            keyFileName = System.Text.Encoding.ASCII.GetString(keyFileBytes, 0, cbBytes - 1);
                        }
                    }
                }
                finally
        ...
        internal extern static bool CryptGetProvParam(IntPtr hCryptProv, 
