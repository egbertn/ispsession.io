...
namespace ispsession.io
{
    public sealed class PersistUtil : Helpers
    {
        ...
        public static byte[] LocalContents(ISPSessionStateItemCollection2 items, out int zLen, bool compress)
        {
        ...
        internal static ISPSessionStateItemCollection2 LocalLoad(Stream pSequentialStream)
        {
            var persistUtil = new PersistUtil(pSequentialStream);

            var retVal = new ISPSessionStateItemCollection2();
            object varTemp;
            string sTemp = persistUtil.ReadProperty(out varTemp);
            if (sTemp != "Els")
            {
                throw new System.Exception("Corrupt Session State");
            }
            var lVDictElements = (int)varTemp;
            if ((lVDictElements < 0) || (lVDictElements > short.MaxValue))
            {
                throw new System.Exception("Too many Session variables");
            }
